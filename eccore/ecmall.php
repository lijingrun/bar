...
{
	static $TAGS = 'LyMall';
	static $log_file = '/temp/logs/lanyu-errors.log';
	public $cur_time;
	public $LEVEL = 0;
	
	static function __get_level_content( $l )
	{
		switch( $l ) {
			default:
			case 0: return 'DEBUG';
	...
	static function log( $c, $t = null, $lv = 0 )
	{
		$log_file = ROOT_PATH.self::$log_file;

		if( empty($t) ) $t = self::$TAGS;
		$content = sprintf( "[%s] <%s> %s: %s\n", 
			date( 'Y-m-d H:i:s',time()), self::__get_level_content( $lv ), $t, $c );
		return 
...
{
	// const variable
	private $SAM_HOST = '120.24.156.120';
	private $SAM_PORT = 1883;
	private $conn;

	public function __construct( $SAM_HOST = null, $SAM_PORT = null ) {
		error_reporting(E_ALL);

		require(ROOT_PATH . '/includes/libraries/mqtt/SAM/php_sam.php');

		if( !empty($SAM_HOST) && !empty($SAM_PORT) ) {
			$this->SAM_HOST = $SAM_HOST;
			$this->SAM_PORT = $SAM_PORT;
		}

		//create a new connection object
		$this->conn = new SAMConnection();

		//start initialise the connection
		$this->conn->connect(SAM_MQTT, 
			array(SAM_HOST => $this->SAM_HOST, SAM_PORT => $this->SAM_PORT));      
	}
	...
	public function send( $target, $message = null )
	{
		if( empty($target) ) {
		...
		$this->conn->send('topic://lanyu/'.$target, $msgCpu);
				 
//		echo 'MQTT Message to ' . $target . ' sent: ' . $message;
	}
...
class Apns
{
	...
	public function __construct() {
		...
		$this->local_cert = ROOT_PATH. '/includes/cert/apns.pem';
		...
		// Sand Box
		//$this->fp = stream_socket_client('ssl://gateway.sandbox.push.apple.com:2195', 
		//	$err, $errstr, 60, STREAM_CLIENT_CONNECT|STREAM_CLIENT_PERSISTENT, $ctx);


		if (!$this->fp) exit("Failed to connect: $err $errstr" . PHP_EOL);
	...
	public function send( $deviceToken, $message = null )
	{
		// lay ipod touch
		//$deviceToken = 'd5dc4bf71b3ca91c53245907482ee3d96e29e5c22ecb5f5cd86eb8808114ec1b';

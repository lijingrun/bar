...
class DefaultApp extends BackendApp
{
    ...
    function welcome()
    {
        ...
        $mobile = $login_record_model->find('terminal = 2');
        $pc = $login_record_model->find('terminal = 1');
        $mobile_login = count($mobile);
        $pc_login = count($pc);
        $total_login = $mobile_login+$pc_login;
        $this->assign('pc_login', $pc_login);
        $this->assign('mobile_login', $mobile_login);
        $this->assign('total_login', $total_login);

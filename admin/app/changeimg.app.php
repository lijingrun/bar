...
class ChangeimgApp extends BackendApp {

    function index() {
        if (!IS_POST) {

            $this->display('change_img.html');
        } else {
//            import('uploader.lib');
            //判断如果上传出现问题，则输出

            if (!empty($_FILES['error'])) {
                show_warning('上传失败，请重新上传！');
            }
            $file = $_FILES['indeximg'];
            //实例化上传插件
            $uploader = new Uploader();
            //设置上传限制
            $uploader->allowed_type('png');
            ...
            //将文件上传到相应的位置url
            $uploader->save('/themes/wapmall/default/styles/default/images','banner_4');
            $this->show_message('上传成功！','点击返回','index.php?app=changeimg');
        }
    }

...
class BackendApp extends ECBaseApp {
    ...
    function login() {
        ...
        if (!IS_POST) {
            ...
            //分开判断，如果是物流管理员，就直接入物流系统，其他就入后台管理系统
            if ($user_id == 14987) {
                $this->show_message('login_successed', 'go_to_admin', 'index.php?app=logistics');
            } else {
    ...
    function _run_action() {
        ...
        //判断，如果是LY物流管理账号并且app=logistics的话，直接跳过权限判断
        if ($_SESSION['admin_info']['user_id'] == 14987 && APP == 'logistics') {
            
        //但是如果是LY物流管理账号，但不是进入物流管理页面的话，直接提示无权限    
        } elseif ($_SESSION['admin_info']['user_id'] == 14987 && APP != 'logistics') {
            $this->show_warning('no_permission','点击返回','index.php?app=logistics');

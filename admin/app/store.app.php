...
class StoreApp extends BackendApp
{
   ...
   function ajax_col()
   {
    ...
    function view()
    {
        ...
        else
        {
            ...
            if (isset($_POST['agree']))
            {
                ...
                $curr_member = $member_model->get('user_id ='.$id);
                if( empty($curr_member['k3_code']) ) {
                    $store_model = & m('store');
                    $store = $store_model->get('store_id ='.$id);
                    $province_id = $this->get_province_id_from_regin_id($store['region_id']);
                    $k3_prefix = $this->generate_k3_code_prefix( $province_id, $store['city_id'] );
                    if( 0 != $k3_prefix ) {
    ...
    function get_province_id_from_regin_id( $region_id ) {
        ...
        $province = $province_model->get("name like '".$region['region_name']."'");
        return $province['id'];
    }
    ...
    //修改K3编码
    function change_k3_code(){
        $id = $_POST['id'];
        $k3_code = $_POST['k3_code'];
        $user_name = $_POST['user_name'];
        $model = & m('anson');
        $k3 = $model->get("fFrowid =".$id);
        $ansons = $model->find("fFBillNO =".$k3['fFBillNO']);
        foreach($ansons as $val):
            $val['fFCustName'] = $user_name;
            $val['fFCustnumber'] = $k3_code;
            $val['fSendstatus'] = 'N';
            $model->edit($val['fFrowid'],$val);
        endforeach;
        echo 111;
        exit;
    ...
    //针对那些没有自动生成K3单的，手动生成
    function add_k3_data(){
        if(IS_POST){
            $order_no = $_POST['order_no'];
            $anson_model = & m('anson');
            $anson = $anson_model->get("fFBillNO like '".$order_no."'");
            if(!empty($anson)){
                echo "记录已经存在，无需生成！";
                exit;
            }
            $order_model = & m('order');
            $order = $order_model->get("order_sn =".$order_no);
            $member_model = & m("member");
            ...
            $this->_order_in_k3($member['k3_code'],$member['user_name'],$order,$k3_pay_status);
            echo 111;
            exit;
        }
    }
    ...
    function k3_orders(){
        ...
        $k3_orders_model = & m('anson');
        //查入唔到的单s
        if(empty($order_no)) {
            $k3_orders = $k3_orders_model->find("fSendstatus = 'N'");
        }else{
            $k3_orders = $k3_orders_model->find("fFBillNO = ".$order_no);
        }
        $this->assign('orders', $k3_orders);
        $this->display('k3_orders.html');
    }

    //取消K3转换器订单的转换
    function cancel_k3_order(){
        $id = $_POST['id'];
        $k3_orders_model = & m('anson');
        $k3_order = $k3_orders_model->get("fFrowid = ".$id);
        if(!empty($k3_order)){
            $orders_to_cancel = $k3_orders_model->find("fFBillNO =".$k3_order['fFBillNO']);
            foreach($orders_to_cancel as $order):
                $order['fSendstatus'] = 'Y';
                $k3_orders_model->edit($order['fFrowid'],$order);
            endforeach;
            echo 111;
        }
        exit;
...
//        $order_id = 123123;
        ...
        $act = $order_act_model->get('Order_id ='.$order_id );
        $order_act_model->drop($act['Act_Id']);
        echo 111;
        exit;
    }

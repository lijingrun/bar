...
(function($, window, document, undefined) {
	...
	var Scroll = $.Class.extend({
		...
		_flick: function(e) {
			...
			if (this.resetPosition(this.options.bounceTime)) {//下拉触发之一
				return;
			}
		...
		_transitionTime: function(time) {
			...
			if (this.options.fixedBadAndorid && !time && $.os.isBadAndroid) {
				this.scrollerStyle['webkitTransitionDuration'] = '#';
				if (this.parallaxElement && this.options.scrollY) { //目前仅支持竖向视差效果
					this.parallaxStyle['webkitTransitionDuration'] = '#';
				}
			}
			if (this.indicators) {
	...
	var Indicator = function(scroller, options) {
		...
		if (this.options.fade) {
			...
			this.wrapperStyle['webkitTransitionDuration'] = this.options.fixedBadAndorid && $.os.isBadAndroid ? '#' : '0ms';
	...
	var PullRefresh = $.Scroll.extend($.extend({
		...
		_scrollbottom: function() {
			if (!this.pulldown && !this.loading) {
				...
				if(this.y-need>=this.options.up.height){
					 this.pullupLoading();
				}
		...
		//API
		resetPosition: function(time) {//关键点
			if (this.pulldown) {
			...
			// 	if (this.y <= this.options.up.height) {

			// 		this.pullupLoading(undefined, time || 0);
			// 		return true;
			// 	} else {
			// 		!this.loading && this.bottomPocket.classList.remove(CLASS_VISIBILITY);
			// 	}
			}

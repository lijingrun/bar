/* spec对象 */
function spec(id, spec1, spec2, price, stock, original_price, point)
{
    this.id = id;
    this.spec1 = spec1;
    this.spec2 = spec2;
...
function goodsspec(specs, specQty, defSpec)
{
    ...
    this.init = function ()
    {
        if (this.specQty >= 1)
        {
            ...
            var pointValues = this.getDistinctValues('point', '');
            console.log(spec1Values);
            
            //默认第一个，不清楚为什么会员登录之后会空，只能这里做 2015-4-22
            if (this.spec1 == null) {
                this.spec1 = spec1Values[0];
            }
            ...
            for (var i = 0; i < spec1Values.length; i++)
            {
                ...
                {
                    $(".handle ul:eq(0)").append("<li class='solid' onclick='selectSpec(1, this);'><a href='javascript:;'><span>" + spec1Values[i]
                            //+ " ( 积分: " + pointValues[i].toString() + " )"	
                            + "</span></a></li>");
                }
                else
                {
                    $(".handle ul:eq(0)").append("<li class='dotted' onclick='selectSpec(1, this)'><a href='javascript:;'><span>" + spec1Values[i]
                            //+ " ( 积分: " + pointValues[i].toString() + " )"	
                            + "</span></a></li>");
                }
            }
        ...
        // Rin: try.
        try {
            console.log(spec.spec1 + ' ' + spec.spec2);
            $("[ectype='current_spec']").html(spec.spec1 + ' ' + spec.point);
        }
        catch (e) {
        }
    }
...
function selectSpec(num, liObj)
{
    ...
    else
    {
        ...
        if (spec != null)
        {
            $("[ectype='current_spec']").html(spec.spec1 + ' ( 积分: ' + spec.point + ' )');
            $("[ectype='goods_price']").html(price_format(spec.price));
            $("[ectype='goods_original_price']").html(price_format(spec.original_price));
...
$(function () {
    ...
    // Rin: auto onclick 2.
    $(".handle ul:eq(0) li").click(
            function () {
                $(".handle ul:eq(1) li:eq(1)").click();
            }
    );

    // Auto click.
    //$('li.dotted').first().click();
    //$(".handle ul:eq(1) li:eq(1)").click();

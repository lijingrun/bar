...
function spec_update(){
    ...
    SPEC.specs && $.each(SPEC.specs,function(i,item){
        ...
        tpl.find('*[item="original_price"]').append('<input type="hidden" name="original_price['+ item.spec_id +']" value="' + item.original_price + '" />' + item.original_price);
        ...
        tpl.find('*[item="point"]').append('<input type="hidden" name="point['+ item.spec_id +']" value="' + item.point+ '" />' + item.point);
        tpl.show();
...
var set_spec_2_select_box = function() {

	/* Rin: 
	 * 		Fix All spec_2 select_box entries.
	 */
	$('*[item="spec_2_select"]').each( function() { 
		var v = $.trim( $(this).prev('[item="spec_2"]').val() );
...
function spec_editor(){
    ...
    /* 规格名称 */
	SPEC.spec_name_1 = '产品规格（型号）';
	SPEC.spec_name_2 = '分级店铺';
	
	// Rin: new style & fixed & readonly title.
	var spec_name_css = {border: 'none', background: 'none', fontSize: '12px', color: '#919191', height: '22px',
		fontFamily: '微软雅黑,Verdana,Arial,sans-serif,"Times New Roman",宋体' };
	
    $('*.[ectype="spec_editor"]').find('*[col="spec_name_1"]').val(SPEC.spec_name_1).attr('readonly','true')
		.css(spec_name_css).parent().addClass('distance1');
    $('*.[ectype="spec_editor"]').find('*[col="spec_name_2"]').val(SPEC.spec_name_2).attr('readonly','true')
		.css(spec_name_css).parent().addClass('distance1');
    ...
    $('*[ectype="save_spec"]').click(function(){
        ...
        $('#dialog_object_spec_editor').find('*[ectype="data"]').each(function(){
			...
			$(this).find('*[item="spec_2"]').val( $(this).find('#sgrade_select').val() );
            var spec_2 = SPEC.spec_name_2 ? $.trim($(this).find('*[item="spec_2"]').val()) : null;
			
            var price = $.trim($(this).find('*[item="price"]').val());
            var original_price = $.trim($(this).find('*[item="original_price"]').val());
            var point= $.trim($(this).find('*[item="point"]').val());

...
class GoodsModel extends BaseModel {
    ...
    function get_list($params = array(), $scate_ids = array(), $desc = false, $no_picture = true) {
        ...
        /* tyioocom */
        $fields .= "g.point_price, g.goods_id, g.store_id, g.type, g.goods_name, g.cate_id, g.cate_name, g.brand, g.spec_qty, g.spec_name_1, g.spec_name_2, g.if_show, g.closed, g.add_time, g.recommended, g.default_image, " .
    ...
    function get_info($id) {
        ...
        if ($goods) {
            ...
            /*
             * 增加判断条件，由于没有判断店铺的审核情况，导致只要申请了店铺，就算未通过都可以查看店铺对应的价钱，
             * 这里增加spec_2=$sgrade的条件，但后台又用到这个查询，当店铺查看后台时候会导致查看不到所有规格，
             * 所以增加条件判断，只有是sgrade为1的人才增加查询条件
             */
            if($sgrade == 1){
            /* 商品规格 */
            ...
            //查找该产品是否参加活动，是的话在活动期间将价格调整为活动价格
            //1.专场活动
            $special_model = & m('special');
           
            
            $specials = $special_model->find('start <' . gmtime() . " AND end >" . gmtime(). " AND sgrade = ".$sgrade);
            //如果当前时间内存在活动专场，就查活动产品表里面是否有对应的spec_id,有就将原来价钱替换成表中价钱
            $special_goods_model = & m('special_goods');
            if (!empty($specials)) {
                foreach ($specials as $special):
                    foreach ($specs as $key => $val):
                        $special_goods = $special_goods_model->get('special_id =' . $special['special_id'] . " AND spec_id =" . $val['spec_id']);
                        if(!empty($special_goods)){
                            $specs[$key]['price'] = $special_goods['price'];
                        }
                    endforeach;
                endforeach;
            }

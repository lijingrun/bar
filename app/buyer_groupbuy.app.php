...
class Buyer_groupbuyApp extends MemberbaseApp
{
    ...
    function index()
    {
        ...
        //新增的团购以量定价界面，搜索有买的，然后显示
        $nums_model = & m('groupbuy_nums');
        $nums_buy_model = & m('groupbuy_nums_buy');
        $nums_buy = $nums_buy_model->find(array(
            'conditions' => "user_id =".$_SESSION['user_info']['user_id'],
        ));
        $nums_ids = array();
        foreach($nums_buy as $val):
            $nums_ids[] = $val['group_id'];
        endforeach;
        $ids_str = implode(',',$nums_ids);
        if(empty($ids_str)){
            $ids_str = 0;
        }
        $nums = $nums_model->find(array(
            'conditions' => 'id in ('.$ids_str.')'
        ));
        $goods_model = & m('goods');
        foreach($nums as $key=>$num):
            ...
            $buy = $nums_buy_model->get('group_id ='.$num['id']." AND user_id =".$_SESSION['user_info']['user_id']);
            $nums[$key]['buy'] = $buy;
    ...
    function exit_group()
    {
        ...
        $this->show_message('exit_groupbuy_succeed'
			/* Rin */
			, Lang::get('go_back'), '?app=buyer_groupbuy'	
		);
    }

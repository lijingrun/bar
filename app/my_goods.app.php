...
class My_goodsApp extends StoreadminbaseApp
{
    ...
    function add()
    {
        ...
        if (!IS_POST)
        {
			 ...
			 /* Rin: Get sgrade from db */
			 $this->assign('user_id', $this->visitor->get('user_id') );
			 $this->assign('sgrades', 
				$this->_sgrade_mod->find(array(
    ...
    function edit()
    {
        ...
        if (!IS_POST)
        {
			...
			/* Rin: Get sgrade from db */
			$this->assign('user_id', $this->visitor->get('user_id') );
			$this->assign('user_sgrade', $this->visitor->get('sgrade') );
			$this->assign('sgrades', 
				$this->_sgrade_mod->find(array(
					'fields' => 'grade_id, grade_name',
				)) 
            ...
            $this->import_resource(array(
                'script' => array(
                    ...
                    array(
                         'path' => 'jquery.ui/jquery-ui-1.8.10.custom.min.js',
                         'attr' => 'charset="utf-8"',
                    ),
                    array(
                         'path' => 'jquery.ui/jquery.ui.datepicker.my.js',
                         'attr' => 'charset="utf-8"',
                    ),
                ),
				'style' => array( 
						array( 'path' => 'jquery.ui/themes/ui-lightness/jquery.ui.css' ), 
						array( 'path' => 'jquery.ui/css/jqueryui/jquery.ui.all.css' ),
				),
...
//            print_r($sgrades);exit
            $this->assign('sgrades', $sgrades);
            //查看本产品的可视级别，并变成数组
            $goods_display = explode(',', $goods['display_sgrade']);
   ...
   function ajax_col()
   {
    ...
    {
        $display_grades = $_POST['display_sgrade'];
        if(empty($display_grades)){
            $this->show_warning("您未选择显示级别，请先选择");
            return;
        }
        $display_grades = implode(',', $display_grades);
        ...
        $goods = array(
            ...
            'views_order'     => $_POST['views_order'],
            'price_distributor'  => $_POST['price_distributor'],
            'point_price' => $_POST['point_price'],
			'tags'             => html_script(trim($_POST['tags'])),
			'ftimes'		   => $_POST['timelist2'],
            'display_sgrade' => $display_grades,
        );
                    ...
                    if ($spec_1)
                    {
                        if (($spec_id = intval($_POST['spec_id'][$key]))) // 已有规格ID的
                        {
                            $specs[$key] = array(
                                ...
                                'original_price'  => $this->_filter_price($_POST['original_price'][$key]),
                                'point'  => $this->_filter_price($_POST['point'][$key]),
    ...
    //检查输入的品牌是否已经存在
    function check_goods_brand(){
        $brand_name = $_GET['brand'];
        $brand_model = & m('brand');
        $store_id = $_SESSION['user_info']['store_id'];
        $brand = $brand_model->get("brand_name like '%".$brand_name."%' AND store_id =".$store_id);

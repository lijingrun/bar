...
class Seller_adminApp extends MemberApp
{
	...
	function _get_member_role()
	{
        ...
        function oem(){
            ...
            if (!IS_POST){
                /* 当前位置 */
                $this->_curlocal(LANG::get('member_center'), 'index.php?app=member', LANG::get('OEM'), 'index.php?app=seller_admin&act=oem', LANG::get('联系方式'));
                ...
                $oem_model->drop($store_id);
                $oem_model->add($new_oem);
                $this->show_message('操作成功！','正在跳转','index.php?app=seller_admin&act=oem');
            }
        }
}

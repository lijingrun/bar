...
define('MIN_STAT_STEP', 10);       // 价格区间最小间隔
...
class SearchApp extends MallbaseApp {
    ...
    function index() {
        ...
        $goods_list = $goods_mod->get_list(array(
            ...
            'order' => isset($_GET['order']) && isset($orders[trim(str_replace('asc', '', str_replace('desc', '', $_GET['order'])))]) ? $_GET['order'] : 'order_by, add_time desc', // tyioocom
    ...
    function _get_list_goods($param, $type = 'recommend') {
        ...
        $data = $goods_mod->find(array(
            ...
            "fields" => "point_price, g.goods_id,default_image,price,goods_name,sales",
            "limit" => $limit
        ));
        ...
        //又是权限的坑爹问题，如果登录了，就按照登录的账号等级将对应的价钱保存入去。。。spec_price
        if(!empty($_SESSION['user_info']['user_id'])){
            $goods_spec_model = & m('goodsspec');
            $sgrade = $_SESSION['user_info']['sgrade'] == 0 ? 1 : $_SESSION['user_info']['sgrade'];
            foreach($data as $key=>$val):
                $goods_spec = $goods_spec_model->get(array(
                    'conditions' => 'spec_2 ='.$sgrade.' AND goods_id ='.$val['goods_id'],
                    'order' => 'price',
                    ));
                if(!empty($goods_spec['price'])){
                    $data[$key]['price'] = $goods_spec['price'];
                }
            endforeach;
        }
//        print_r($data);exit;
    ...
    function _get_goods_conditions($param) {
        ...
        if($_SESSION['user_info']['sgrade'] == 1 || empty($_SESSION['user_info']['sgrade'])){
            if (isset($param['price'])) {
                $min = $param['price']['min'];
                $max = $param['price']['max'];
            ...
            if (isset($param['price'])) {
                $min = $param['price']['min'];
                $max = $param['price']['max'];
                $min > 0 && $conditions .= " AND g.price_distributor >= '$min'";
                $max > 0 && $conditions .= " AND g.price_distributor <= '$max'";
            }
        }
        // sku tyioocom 
        if (isset($param['props'])) {
    ...
    function _get_group_by_info($param, $cached) {
        $data = false;
        $cached = false; //由于经销商和消费者的不一样，这里先不用数据缓存
        if ($cached) {
        ...
        if ($data === false) {
            ...
            if ($total_count > 0) {
                ...
                if($_SESSION['user_info']['sgrade'] == 1 || empty($_SESSION['user_info']['sgrade'])){
                    ...
                    if ($total_count > NUM_PER_PAGE) {
                        $sql = "SELECT MIN(g.price_distributor) AS min, MAX(g.price_distributor) AS max FROM {$table} WHERE" . $conditions;
                        $row = $goods_mod->getRow($sql);
                        $min = $row['min'];
                        $max = min($row['max'], MAX_STAT_PRICE);
                        $step = max(ceil(($max - $min) / PRICE_INTERVAL_NUM), 5);
                        $sql = "SELECT FLOOR((g.price_distributor - '$min') / '$step') AS i, count(*) AS count FROM {$table} WHERE " . $conditions . " GROUP BY i ORDER BY i";
                        $res = $goods_mod->db->query($sql);
                        while ($row = $goods_mod->db->fetchRow($res)) {
                            $data['by_price'][] = array(
                                'count' => $row['count'],
...
//                $pvs = recommend_goodsstr($pvs, 1); // 去掉前面的";"
                $props_data = array();  //产品附加属性
                if (!empty($pvs)) {
                    ...
                    foreach ($pv_arr as $key => $pv) {
                        ...
                        if (!empty($item[1]) && !in_array($item[0], $p)) { //  如果参数已经筛选过了，那么就屏蔽掉。
                            ...
                            $props = $props_mod->get(array('conditions' => 'status=1 and pid=' . $item[0], 'order' => 'sort_order', 'fields' => 'name,pid'));

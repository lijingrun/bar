...
class FrontendApp extends ECBaseApp {
    ...
    function login() {
        ...
        if (!IS_POST) {
            ...
            $user_name = $_COOKIE['ecm_username'];
            $password = $_COOKIE['ecm_password'];
            ...
            setcookie('ecm_password',$password);
            setcookie('ecm_username',$user_name);
            $ms = & ms();
    ...
    function _do_login($user_id) {
        //运行登录的时候记录客户登录终端
        $login_record_model = & m('login_record');
        $terminal = $this->_get_terminal();
        $new_login_record = array(
            'terminal' => $terminal,
            'time' => time(),
            'ip' => real_ip(),
            'user_id' => $user_id,
        );
        $login_record_model->add($new_login_record);
        //如果有openid，登录时候就直接绑定openid
        if(!empty($_SESSION['weixin']['openid'])){
            $member_model = & m('member');
            $member_model->edit($user_id,"openid ='".$_SESSION['weixin']['openid']."'");
        }
        ...
        $user_info = $mod_user->get(array(
            ...
            'fields' => 'user_id, user_name, reg_time, last_login, last_ip, store_id, sgrade, point',
        ));
        //再次判断店铺是否通过审核，不是正常店铺就将sgrade变为空
        $store_model = & m('store');
        $store = $store_model->get('state = 1 AND store_id = '.$user_id);
        if(empty($store['store_id'])){
            $user_info['sgrade'] = 0;
        }
        //判断当天是否第一次登陆
//        $first_login = false;
//        $last_login = $user_info['last_login'] + 28800; //上次登陆时间+8小时
//        $today = strtotime(date('Y-m-d', time()));
//        //如果上次登陆时间小于今日凌晨，证明今次的登陆时间是今天第一次
//        if ($last_login < $today) {
//            $first_login = true;
//        }
        ...
        /* 更新用户登录信息 如果是第一次登陆，加1分，否则，不加 */ //2015-12-30取消登录送积分
//        if (!$first_login) {
//            $mod_user->edit("user_id = '{$user_id}'", "last_login = '" . gmtime() . "', last_ip = '" . real_ip() . "', logins = logins + 1");
//        } else {
//            $mod_user->edit("user_id = '{$user_id}'", "last_login = '" . gmtime() . "', last_ip = '" . real_ip() . "', logins = logins + 1 , point = point + 1, login_point = login_point + 1");
//        }
        /* 更新购物车中的数据 */
        $mod_cart = & m('cart');
...
class UserVisitor extends BaseVisitor {
    ...
    function logout() {
		...
		// remove open_id when logout.
		$member = & m('member');
		if(!empty($_SESSION['weixin']['openid'])){
			$member->edit("openid ='".$_SESSION['weixin']['openid']."'", array( 'openid' => '', ));
		}
...
class ShoppingbaseApp extends MallbaseApp {
    ...
    function _run_action() {
        /* 只有登录的用户才可访问 */
        if (!$this->visitor->has_login && !in_array(ACT, array('login', 'register', 'check_user', 'fast_login'))) {
            if (!IS_AJAX) {
                header('Location:index.php?app=member&act=login&ret_url=' . rawurlencode($_SERVER['PHP_SELF'] . '?' . $_SERVER['QUERY_STRING']));
    ...
    function _get_member_menu() {
        ...
        $menu['my_ecmall'] = array(
            ...
            'submenu' => array(
                ...
                'article' => array(
                    'text' => '升级公告',
                    'url' => 'index.php?app=article&cate_id=4',
                    'name' => 'friend',
                    'icon' => 'ico5',
                )
            /*
...
//            $menu['im_seller']['submenu']['my_protect'] = array(
//                'text' => '品牌保护',
...
//                'icon' => 'ico22',
//            );
//            $menu['im_seller']['submenu']['groupbuy_nums'] = array(
//                'text' => '以量定价',
//                'url' => 'index.php?app=groupbuy_nums',
//                'name' => 'groupbuy_nums',
//                'icon' => 'ico22',
//            );
//            $menu['im_seller']['submenu']['promotion'] = array(
//                'text' => '限时促销',
//                'url' => 'index.php?app=promotion',
//                'name' => 'promotion',
//                'icon' => 'ico20',
//            );
//            $menu['im_seller']['submenu']['special'] = array(
//                'text' => '活动专场',
//                'url' => 'index.php?app=special',
//                'name' => 'special',
//                'icon' => 'ico8',
//            );
//			$menu['im_seller']['submenu']['fortune_wheel'] = array(
//                'text' => '幸运大转盘',
...
//            );
            ...
            $menu['im_seller']['submenu']['order_rule'] = array(
                'text' => Lang::get('order_rule'),
                'url' => 'index.php?app=order_rule',
                'name' => 'order_rule_manage',
                'icon' => 'ico10',
            );
            $menu['im_seller']['submenu']['order_rule'] = array(
                'text' => Lang::get('order_rule'),
...
//            $menu['im_seller']['submenu']['mount_policy'] = array(
//                'text' => '大姨妈政策',
//                'url' => 'index.php?app=mount_policy',
//                'name' => 'mount_policy',
//                'icon' => 'ico10',
//            );
...
//                'text' => '加盟介绍',
//                'url' => 'index.php?app=join',
//                'name' => 'join',
//                'icon' => 'ico11',
//            );
//            $menu['im_seller']['submenu']['oem'] = array(
//                'text' => 'OEM',
//                'url' => 'index.php?app=seller_admin&act=oem',
//                'name' => 'oem',
//                'icon' => 'ico11',
//            );
//            $menu['im_seller']['submenu']['enterprise'] = array(
//                'text' => '企业简介',
//                'url' => 'index.php?app=introduction',
//                'name' => 'introduction',
//                'icon' => 'ico11',
//            );
//            $menu['im_seller']['submenu']['edition'] = array(
//                'text' => '版本展示',
//                'url' => 'index.php?app=edition&act=edition_list',
//                'name' => 'edition',
//                'icon' => 'ico5',
//            );
...
class StorebaseApp extends FrontendApp {
    ...
    function get_store_data() {
        ...
        if ($store === false) {
            ...
            if (empty($store)) {
                $this->show_warning('店铺正在筹备装修，敬请期待');
                exit;
            }
            if ($store['state'] == 2) {

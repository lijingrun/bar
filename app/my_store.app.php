...
class My_storeApp extends StoreadminbaseApp {
        ...
        if (!IS_POST) {
            ...
            if ($data === false) {
                ...
                if ($store['store_code'] != '' && $data['store_code'] != '') {
                    $store_code_old = pathinfo($store['store_code']);
                    $store_code_new = pathinfo($data['store_code']);
                    if ($store_code_old['extension'] != $store_code_new['extension']) {
                        unlink($store['store_code']);
                    }
                }
    ...
    function _upload_files() {
        ...
        if ($file['error'] == UPLOAD_ERR_OK && $file != '') {
            ...
            $uploader->root_dir(ROOT_PATH);
            $data['store_code'] = $uploader->save('data/files/store_' . $this->_store_id . '/other', 'store_code');
        }
    ...
    //qq多客服设置 
    function qq_services(){
        /* 当前位置 */
        $this->_curlocal(LANG::get('member_center'), 'index.php?app=my_store', LANG::get('店铺设置'), 'index.php?app=my_stoer', LANG::get('qq多客服'));
        /* 当前用户中心菜单 */
        $type = (isset($_GET['type']) && $_GET['type'] != '') ? trim($_GET['type']) : 'all_orders';
        $this->_curitem('qq多客服');    //默认选择项，填写frontent.besc里面菜单列表对应的name项
        $this->_curmenu($type);
        $this->_config_seo('title', Lang::get('member_center') . ' - ' . Lang::get('qq多客服'));

        $store_id = $_SESSION['user_info']['store_id'];
        if(empty($store_id)){
            $this->show_warning('对不起，你还没有开店！','请先开店！','index.php');
            return;
        }
        $store_model = & m('store');
        ...
        if(!IS_POST){
            ...
            $qqs = $qq_service_model->findAll();
            $this->assign('qqs', $qqs);
            $this->display('qq_services.html');
        }else{
            
            $region_ids = $_POST['region_id'];
            $region_names = $_POST['region_name'];
            $qqs = $_POST['qq'];
            $phones = $_POST['phone'];
            $count = count($region_ids);
            //不过之前有无设置过了，直接就清楚店铺下面对应的所有客服qq记录，重新加入！
            $qq_service_model->drop('store_id ='.$store_id);
//            echo 111;exit;
            //循环加入
            for($i = 0; $i < $count; $i++):
                $new_qq = array(
                ...
                $qq_service_model->add($new_qq);
            endfor;
            $this->show_message('操作成功！','保存成功！','index.php?app=my_store&act=qq_services');
        }
    }

...
class My_wxmenuApp extends StoreadminbaseApp {
    ...
    private function getmenu() {
        ...
        $topmemu = $this->my_wxmenu_mod->find(array(
            'fields' => 'id, name, weixin_key as wkey, weixin_keyword as keyword, weixin_type, likes ',
    ...
    function add() {
        ...
        if (!IS_POST) {
            ...
            $data = array(
                ...
                'weixin_keyword' => empty($_POST['keyword'])?'keyword':trim($_POST['keyword']),
                'weixin_key' => empty($_POST['key'])?'key':trim($_POST['key']),
                'likes' => trim($_POST['likes']),
                'user_id' => $this->_store_id
            );
            if ($this->my_wxmenu_mod->name_exists($data['name'], $data['pid'],$this->_store_id)) {
    ...
    function edit() {
        ...
        if (!IS_POST) {
            ...
            if ($this->my_wxmenu_mod->name_exists($data['name'], $data['pid'],$this->_store_id, $id)) {
                Init_Weixin::ajaxReturn(0, '分类已经存在');
            }
